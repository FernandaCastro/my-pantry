...
@SpringBootTest(classes = {GroupMemberCacheService.class})
...
public class GroupMemberCacheServiceIT {
    ...
    @Autowired
    private GroupMemberCacheService memberCacheService;
    ...
    void givenEmptyCache_whenGetFromCache2Times_thenMissCacheThenHitCache() {
        ...
        var cacheList = List.of(
                GroupMemberCacheDto.builder().accountGroupId(1L).roleId("OWNER").build()
        );
        when(memberRepository.findAllByEmail(anKey)).thenReturn(databaseList);

        //when
        List<GroupMemberCacheDto> cacheMiss = memberCacheService.getFromCache(anKey);
        List<GroupMemberCacheDto> cacheHit = memberCacheService.getFromCache(anKey);
    ...
    void givenCacheWithOneKey_whenUpdateCache_thenAddToValues() {
        ...
        var givenMember = GroupMemberCacheDto.builder().accountGroupId(1L).roleId("OWNER").build();
        List<GroupMemberCacheDto> givenCacheValues = new ArrayList<>();
        ...
        //then
        List<GroupMemberCacheDto> updateCache = new ArrayList();
        updateCache.add(givenMember);
        updateCache.add(GroupMemberCacheDto.builder()
                .accountGroupId(anAccountGroup.getId())
                .roleId(anRole)
                .build());
        verify(memberCache, times(1)).put(anKey, updateCache);

        var finalCache = (List<GroupMemberCacheDto>) memberCache.get(anKey).get();
    ...
    void givenEmptyCache_whenUpdateCache_thenDoNothing() {
        ...
        var newMember = GroupMemberCacheDto.builder()
                .accountGroupId(anAccountGroup.getId())
                .roleId(anRole)
                .build();

        var updatedCacheValuees = new ArrayList<GroupMemberCacheDto>();
    ...
    void givenCacheWithOneKey_whenDeleteFromCache_thenRemoveOneItemFromValues() {
        //given
        //Load given data to the Cache
        var givenMember1 = GroupMemberCacheDto.builder().accountGroupId(1L).roleId("OWNER").build();
        var givenMember2 = GroupMemberCacheDto.builder().accountGroupId(2L).roleId("ADMIN").build();
        ...
        //then
        List<GroupMemberCacheDto> updateCacheValues = new ArrayList();
    ...
    void givenCacheWithTwoKeys_whenDeleteAllFromCache_thenRemoveAllKeysOrValuesReferringAGroup() {
        ...
        var givenAndKept = GroupMemberCacheDto.builder().accountGroupId(1L).roleId("OWNER").build(); //Expected to be the only entry left in cache
        var givenAndDeleted1 = GroupMemberCacheDto.builder().accountGroupId(group2).roleId("ADMIN").build(); //Expected to be removed from cache
        var givenAndDeleted2 = GroupMemberCacheDto.builder().accountGroupId(group2).roleId("OWNER").build(); //Expected to be removed from cache

        List<GroupMemberCacheDto> givenValuesKey1 = new ArrayList<>();
        givenValuesKey1.add(givenAndKept);
        givenValuesKey1.add(givenAndDeleted1);
        memberCache.put(anKey, givenValuesKey1);

        List<GroupMemberCacheDto> givenValuesKey2 = new ArrayList<>();
        ...
        var finalCacheEmail1 = (List<GroupMemberCacheDto>) memberCache.get(anKey).get();
